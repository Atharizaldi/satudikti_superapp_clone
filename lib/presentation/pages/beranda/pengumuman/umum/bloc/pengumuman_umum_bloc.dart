...
part 'pengumuman_umum_event.dart';
part 'pengumuman_umum_state.dart';

class PengumumanUmumBloc
    extends Bloc<PengumumanUmumEvent, PengumumanUmumState> {
  ...
  PengumumanUmumBloc(
    ...
    on<GetPengumumanEvent>(_getPengumuman);
  }

  void _getPengumuman(
      GetPengumumanEvent event, Emitter<PengumumanUmumState> emit) async {
    ...
    if (hasConnection) {
      if (state is PengumumanLoadedState) {
        ...
        final result = await getPengungumanWithCategory.execute(
            '1', defaultLimit.toString(), kategori1);
