...
class AuthorizationCodeGrant {
  ...
  Uri getAuthorizationUrl(Uri redirect,
    ...
    print("+++ ASCII ENCODE: ${ascii.encode(_codeVerifier)}");
    print("+++ shaconvert ${sha256.convert(ascii.encode(_codeVerifier))}");
    print(
        "+++ shaconvert bytes ${sha256.convert(ascii.encode(_codeVerifier)).bytes}");
    print(
        "+++ base64url encode ${base64Url.encode(sha256.convert(ascii.encode(_codeVerifier)).bytes)}");
    ...
    //     "+++ shaconvert bytes ${sha256.convert(ascii.encode(_codeVerifier)).bytes}");
    print(
        "+++ base64 encode ${base64.encode(sha256.convert(ascii.encode(_codeVerifier)).bytes)}");

    // print("+++ UTF8 ENCODE: ${utf8.encode(_codeVerifier)}");
    // print("+++ utf8 shaconvert ${sha256.convert(utf8.encode(_codeVerifier))}");
    // print(
    //     "+++ utf8 shaconvert bytes ${sha256.convert(utf8.encode(_codeVerifier)).bytes}");
    // print(
    //     "+++ utf8 base64 encode ${base64Url.encode(sha256.convert(utf8.encode(_codeVerifier)).bytes)}");
    ...
    //     .replaceAll(RegExp('_'), '/');
    var codeChallenge =
        base64.encode(sha256.convert(ascii.encode(_codeVerifier)).bytes);
  ...
  Future<Client> _handleAuthorizationCode(String? authorizationCode) async {
    ...
    // Uri uriRequest = Uri.parse(
    //     "${tokenEndpoint}?grant_type=authorization_code&client_id=$identifier&client_secret=$secret&redirect_uri=${_redirectEndpoint.toString()}&code=$authorizationCode&code_verifier=${_codeVerifier!}");
    // print("URIREQUEST $uriRequest");
    var response =
        await _httpClient!.post(tokenEndpoint, body: body, headers: headers);
    ...
    print("Credentials expiration ${credentials.expiration}");
    print("Credentials refreshToken ${credentials.refreshToken}");
    print("Credentials idToken ${credentials.idToken}");
    print("Credentials scopes ${credentials.scopes}");
    print("Credentials scopes ${credentials.scopes}");
